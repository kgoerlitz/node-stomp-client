...
StompFrameEmitter.prototype.parseBody = function () {
  ...
  if (this.frame.contentLength > -1) {
    if (this.frame.contentLength <= Buffer.byteLength(this.buffer)) {
      var bufferBuffer = new Buffer(this.buffer);
      this.frame.appendToBody(bufferBuffer.slice(0, this.frame.contentLength).toString());
      this.buffer = bufferBuffer.slice(this.frame.contentLength, bufferBuffer.length).toString();
