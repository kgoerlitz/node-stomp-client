...
StompClient.prototype.onConnect = function() {
  ...
  frameEmitter.on('MESSAGE', function(frame) {
    var subscription = frame.headers.subscription || frame.headers.destination;
    var subscribed = self.subscriptions[subscription];
  ...
  frameEmitter.on('CONNECTED', function(frame) {
    if (self._retryNumber > 0) {
      ...
      self.emit('connect', frame.headers.session, frame.headers);
    }
  });
  ...
  var headers = {
    'accept-version': (this.version === '1.2') ? '1.0,1.1,1.2' : (this.version === '1.1') ? '1.0,1.1' :'1.0',
    'login': self.user,
    'passcode': self.pass
  };

  if(this.vhost && this.version !== '1.0')
    headers.host = this.vhost;

  var frame = new StompFrame({
    command: (this.version !== '1.0') ? 'STOMP' : 'CONNECT',
    headers: headers
  }).send(self.stream);

  //if we've just reconnected, we'll need to re-subscribe
  for (var subscription in self.subscriptions) {
    new StompFrame({
      command: 'SUBSCRIBE',
      headers: self.subscriptions[subscription].headers
    }).send(self.stream);
  }
};
...
StompClient.prototype.subscribe = function(queue, _headers, _callback) {
  ...
  var subscription = headers.id || queue;
  ...
  this.subscriptions[subscription].listeners.push(callback);
  return this;
};
...
function sendAckNack(acknack, messageId, subscription, transaction) {
  var headers = (this.version === '1.2') ? {'id': messageId} : {'message-id': messageId, 'subscription': subscription};
  if(transaction) {
